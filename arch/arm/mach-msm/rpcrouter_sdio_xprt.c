...
#define IS_RPC_DBG (sdio_dbg_flag & DBG_RPC)
...
#if defined(CONFIG_MSM_RPC_SDIO_DEBUG) /* HTC: add IS_RPC_DBG checking */
#define SDIO_XPRT_DBG(x...) do {                \
	if (msm_sdio_xprt_debug_mask & MSM_SDIO_XPRT_DEBUG || IS_RPC_DBG)     \
		printk(KERN_DEBUG x);           \
	} while (0)

#define SDIO_XPRT_INFO(x...) do {               \
	if (msm_sdio_xprt_debug_mask & MSM_SDIO_XPRT_INFO || IS_RPC_DBG)      \
		printk(KERN_INFO x);            \
	} while (0)
#else
