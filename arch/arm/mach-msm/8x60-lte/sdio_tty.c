...
static int sdio_tty_open(struct tty_struct *tty, struct file *f)
{
	int res = 0;
	int n = tty->index;
/* RIL: Mars@20110614: since we only can call sdio_open once, so we save the open status in a gobal paramters. */
	int nOpenFlag = 0;
/* SSD_RIL */
	...
	if (info->sdio_open == 0) {
		info->tty_open_count++;
	/* HTC_CSP:Since the limitation of sdio_al(The channel can be opened once), we will keep the channel info if it's ever opened */
		strncpy(wake_lock_name, name, 6); /*SDIO_X*/
		wake_lock_name[6] = '\0';
...
static int __init sdio_tty_init(void)
{
	...
	/* Do not enable sdio_tty in vigor normal mode.
	 * It makes USB-DUN can't work
	 */
	if (machine_is_vigor() && board_mfg_mode() == 0)
		return 0;
