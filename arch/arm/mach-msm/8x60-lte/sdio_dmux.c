...
/* HTC */
#define IS_DMUX_DBG (sdio_dbg_flag & DBG_DMUX)
...
#if defined(DEBUG) /* HTC: add IS_DMUX_DBG checking */
...
#define DBG(x...) do {		                 \
		if (msm_sdio_dmux_debug_enable || IS_DMUX_DBG)  \
			pr_debug(x);	         \
	} while (0)
...
static void sdio_mux_affinity(struct work_struct *work); /* HTC */
...
static DECLARE_DELAYED_WORK(delayed_work_sdio_mux_affinity, sdio_mux_affinity); /* HTC */
...
static int sdio_dmux_probe(struct platform_device *pdev)
{
	...
	if (!sdio_mux_initialized) {
		...
		/* HTC: Lloyd for throughput */
		queue_delayed_work(sdio_mux_workqueue,  &delayed_work_sdio_mux_affinity,
