...
static struct audience_ctl_ops default_ctl_ops;
static struct audience_ctl_ops *ctl_ops = &default_ctl_ops;
...
void a1026_register_ctl_ops(struct audience_ctl_ops *ops)
{
	ctl_ops = ops;
}
...
static void a1026_i2c_hw_reset(void)
{
	if (ctl_ops->hw_reset) {
		ctl_ops->hw_reset();
		return;
...
static ssize_t a1026_bootup_init(struct file *file, struct a1026img *img)
{
	...
	if (img->img_size > A1026_MAX_FW_SIZE) {
		pr_aud_err("%s: invalid a1026 image size %d\n", __func__,
			img->img_size);
		return -EINVAL;
	}
	...
	if (copy_from_user(vp->data, img->buf, img->img_size)) {
		pr_aud_err("%s: copy from user failed\n", __func__);
		kfree(vp->data);
		return -EFAULT;
	}
	...
	while (retry--) {
		...
		if (rc < 0) {
			pr_aud_err("%s: set boot mode error \
				(%d retries left)\n", __func__, retry);
			continue;
		}
		...
		if (rc < 0) {
			pr_aud_err("%s: boot mode ack error \
				(%d retries left)\n", __func__, retry);
			continue;
		}

		if (buf[0] != A1026_msg_BOOT_ACK) {
			pr_aud_err("%s: not a boot-mode ack \
				(%d retries left)\n", __func__, retry);
			continue;
		}
		...
		pr_aud_info("%s: starting to load image (%d passes)...\n",
			__func__,
			remaining + !!(vp->img_size % 32));
		...
		if (rc < 0) {
			pr_aud_err("%s: fw load error %d (%d retries left)\n",
				__func__, rc, retry);
			continue;
		}
		...
		pr_aud_info("%s: firmware loaded successfully\n", __func__);

		rc = execute_cmdmsg(A100_msg_Sync);
		if (rc < 0) {
			pr_aud_err("%s: sync command error %d \
				(%d retries left)\n", __func__, rc, retry);
			continue;
		}
...
int a1026_filter_vp_cmd(int cmd, int mode)
{
	...
	pr_aud_info("%s: %x filtered = %x, a1026_NS_state %d, mode %d\n",\
			__func__, cmd, filtered_cmd, a1026_NS_state, mode);
...
int a1026_set_config(int newid, int mode)
{
	...
	case A1026_PATH_INCALL_BT:
		if (ctl_ops->support_bt)
			if (!ctl_ops->support_bt()) {
				pr_aud_err("HOY a1026 not support bt.");
				return rc;
			}
		a1026_gpio_set_value(pdata->gpio_a1026_micsel, 0);
	...
	if (a1026_cmds_len > 0) {
		int cmd_size = build_cmds(custom_cmds, newid);
		if (cmd_size > 0)
			i2c_cmds = custom_cmds;
			size = cmd_size;
			pr_aud_info("use customize command\n");
	}

	pr_aud_info("%s: change to mode %d\n", __func__, newid);

	pr_aud_info("%s: block write start (size = %d)\n", __func__, size);

#if DEBUG
	{
		int i = 0;
		for (i = 1; i <= size; i++) {
			pr_aud_info("%x ", *(i2c_cmds + i - 1));
			if (!(i % 4))
				pr_aud_info("\n");
		}
	}
	...
	if (rc < 0) {
		pr_aud_err("A1026 CMD block write error!\n");
		a1026_i2c_sw_reset(sw_reset);
		return rc;
	}
	pr_aud_info("%s: block write end\n", __func__);
	...
	msleep(20);
	pr_aud_info("%s: CMD ACK block read start\n", __func__);
	rc = a1026_i2c_read(ack_buf, size);
	if (rc < 0) {
		pr_aud_err("%s: CMD ACK block read error\n", __func__);
		a1026_i2c_sw_reset(sw_reset);
		return rc;
	} else {
		pr_aud_info("%s: CMD ACK block read end\n", __func__);
#if DEBUG
	{
		int i = 0;
		for (i = 1; i <= size; i++) {
			pr_aud_info("%x ", ack_buf[i-1]);
			if (!(i % 4))
				pr_aud_info("\n");
		}
	}
...
rd_retry:
...
#if DEBUG
				{
					...
					pr_aud_err("%s: CMD ACK Not Ready\n",
						__func__);
					return -EBUSY;
				}
...
int execute_cmdmsg(unsigned int msg)
{
	...
	while (retries--) {
		...
		if (rc < 0) {
			pr_aud_err("%s: ack-read error %d (%d retries)\n",\
				__func__, rc, retries);
			continue;
		}
		...
		if (msgbuf[0] == 0x80  && msgbuf[1] == chkbuf[1]) {
			...
			pr_aud_err("%s: illegal cmd %08x\n", __func__, msg);
			rc = -EINVAL;
			break;
		} else if (msgbuf[0] == 0x00 && msgbuf[1] == 0x00) {
			pr_aud_info("%s: not ready (%d retries)\n", __func__,
				retries);
			rc = -EBUSY;
		} else {
			pr_aud_info("%s: cmd/ack mismatch: (%d retries left)\n",
				__func__,
				retries);
#if DEBUG
		pr_aud_info("%s: msgbuf[0] = %x\n", __func__, msgbuf[0]);
		pr_aud_info("%s: msgbuf[1] = %x\n", __func__, msgbuf[1]);
		pr_aud_info("%s: msgbuf[2] = %x\n", __func__, msgbuf[2]);
		pr_aud_info("%s: msgbuf[3] = %x\n", __func__, msgbuf[3]);
#endif
			rc = -EBUSY;
		}
	}
	...
	if (!pass) {
		pr_aud_err("%s: failed execute cmd %08x (%d)\n", __func__,
			msg, rc);
		a1026_i2c_sw_reset(sw_reset);
	}
	return rc;
...
static int a1026_set_mic_state(char miccase)
{
	...
	if (ctl_ops->set_mic_state)
		return ctl_ops->set_mic_state(miccase);
	   ...
	   unsigned long arg)
{
	...
	case A1026_SET_CONFIG:
		...
		if (rc < 0)
			pr_aud_err("%s: A1026_SET_CONFIG (%d) error %d!\n",
				__func__, pathid, rc);
		break;
	case A1026_SET_NS_STATE:
		if (copy_from_user(&ns_state, argp, sizeof(ns_state)))
			return -EFAULT;
		pr_aud_info("%s: set noise suppression %d\n", \
			__func__, ns_state);
	...
	case A1026_SET_PARAM:
		...
		if (cfg.data_len <= 0 || cfg.data_len > PARAM_MAX) {
				pr_aud_err("%s: invalid data length %d\n", \
					__func__,  cfg.data_len);
				return -EINVAL;
		}

		if (cfg.cmd_data == NULL) {
			pr_aud_err("%s: invalid data\n", __func__);
			return -EINVAL;
		}
		...
		if (copy_from_user(config_data, cfg.cmd_data, cfg.data_len)) {
			pr_aud_err("%s: copy data from user failed.\n",\
				__func__);
			kfree(config_data);
			config_data = NULL;
			return -EFAULT;
		...
		pr_aud_info("%s: update a1026 i2c commands success.\n",\
			__func__);
		rc = 0;
		break;
#if ENABLE_DIAG_IOCTLS
	case A1026_SET_MIC_ONOFF:
		...
		if (rc < 0)
			pr_aud_err("%s: A1026_SET_MIC_ONOFF %d error %d!\n",
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (pdata == NULL) {
		...
		if (pdata == NULL) {
			rc = -ENOMEM;
			pr_aud_err("%s: platform data is NULL\n", __func__);
	...
	if (rc < 0) {
		pr_aud_err("%s: request clk gpio direction failed\n", __func__);
...
chk_gpio_micsel:
	...
	if (rc < 0) {
		pr_aud_info("%s: gpio request mic_sel pin failed\n", __func__);
	} else {
		rc = gpio_direction_output(pdata->gpio_a1026_micsel, 1);
		if (rc < 0) {
			pr_aud_err("%s: request mic_sel gpio direction \
				failed\n", __func__);
	...
	if (rc < 0) {
		pr_aud_err("%s: gpio request wakeup pin failed\n", __func__);
		goto err_free_gpio;
	}

	rc = gpio_direction_output(pdata->gpio_a1026_wakeup, 1);
	if (rc < 0) {
		pr_aud_err("%s: request wakeup gpio direction failed\n",\
			__func__);
		goto err_free_gpio;
	}
	...
	if (rc < 0) {
		pr_aud_err("%s: gpio request reset pin failed\n", __func__);
		goto err_free_gpio;
	}

	rc = gpio_direction_output(pdata->gpio_a1026_reset, 1);
	if (rc < 0) {
		pr_aud_err("%s: request reset gpio direction failed\n",\
			__func__);
		goto err_free_gpio_all;
	}

	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		pr_aud_err("%s: i2c check functionality error\n", __func__);
		rc = -ENODEV;
		goto err_free_gpio_all;
	}
	...
	if (rc) {
		pr_aud_err("%s: a1026_device register failed\n", __func__);
		goto err_free_gpio_all;
	}
